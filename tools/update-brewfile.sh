...
kde_tap_dir="$(brew --repo bomberfish/kde)"
...
for formula in "${formula_dir}"/*.rb; do
  ...
  formula_name=${formula_file//\.rb/}
    if grep -q -E -l 'url "http|url "file' "${formula}"; then
      echo "brew \"bomberfish/kde/${formula_name}\"" >> "${bundle}"
    else
      echo "brew \"bomberfish/kde/${formula_name}\", args: [\"HEAD\"]" >> "${bundle}"
    fi
done
...
for cask in "${casks_dir}"/*.rb; do
  ...
  echo "cask \"bomberfish/kde/${cask_name}\"" >> "${bundle}"
done
